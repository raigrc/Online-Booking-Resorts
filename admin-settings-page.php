...
if (!isset($_SESSION['loggedAdmin'])) {
    header('Location: admin-login-page.php');
                        ...
                        $id = $_GET['id'];
                        $rows = mysqli_fetch_assoc(mysqli_query($mysqli, "SELECT * FROM admins where EmailAddress = '$id'"));
                    ?>
            ...
            <div class="update-content">
                <div class="content-header">
                    <div class="header-left">
                        <h1><?php echo $_SESSION['loggedAdmin'] ?></h1>
                    </div>
                ...
                <input type="hidden" id="admin-id" value="<?php echo $rows['UserID']?>">
                    <div class="form-item">
                        <label for="fname-add">First Name</label>
                        <input type="text" name="fname-add" id="fname-add" value="<?php echo $rows['FirstName'] ?>">
                    </div>
                    <div class="form-item">
                        <label for="lname-add">Last Name</label>
                        <input type="text" name="lname-add" id="lname-add" value="<?php echo $rows['LastName'] ?>">
                    </div>
                    <div class="form-item">
                        <label for="email-add">Email Address</label>
                        <input type="email" name="email-add" id="email-add" value="<?php echo $rows['EmailAddress'] ?>">
                    </div>
                    <div class="form-item">
                        <label for="pword-add">Password</label>
                        <input type="text" name="pword-add" id="pword-add" value="<?php echo $rows['Password'] ?>">
                    </div>
                    <div class="form-item">
                        <label for="number-add">Mobile Number</label>
                        <input type="text" name="number-add" id="number-add" value="<?php echo $rows['MobileNumber'] ?>">
                    </div>

                    <button type="button" onclick="submitDataAdmin('insertadmin')">Insert</button>
                </form>
            </div>
        </section>
    </div>
